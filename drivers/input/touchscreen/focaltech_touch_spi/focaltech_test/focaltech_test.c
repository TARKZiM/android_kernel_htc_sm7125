...
static int fts_test_save_test_data(char *file_name, char *data_buf, int len)
{
    ...
    snprintf(filepath, FILE_NAME_LENGTH, "%s%s", FTS_CSV_FILE_PATH, file_name);
    if (NULL == pfile) {
...
static int fts_test_entry(char *ini_file_name)
{
    ...
    if (true == fts_test_start()) {
        pr_err("\n\n=======Tp test pass.\n");
        fts_data->test_result = true;
    } else {
        pr_err("\n\n=======Tp test failure.\n");
        fts_data->test_result = false;
    }
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    //fwname[count - 1] = '\0';
    //FTS_TEST_DBG("------fts_test_show------fwname:%s.", fwname);
	...
	if(true == ts_data->test_result) {
		FTS_ERROR("fts test tp %s result=%d\n",__func__,ts_data->test_result);
