...
/* BEGIN, Ontim,  wzx, 2020/03/25, St-result :PASS,LCD and TP Device information */
extern u8 fw_file[];
/** add TP info **/
extern char lcd_info_pr[255];
#include <ontim/ontim_dev_dgb.h>
static char version[40]="0x00";
static char vendor_name[50]="tcl_ft8756";
static char lcdname[50]="ft8756";
...
*****************************************************************************/
struct ic_key_value ic_key_value_array []=
{
	{"FT8716",0x87160805},
	{"FT8756",0x87560815},
	{"FT8719",0x8719080D},
...
static void get_ic_chip_type(struct fts_ts_data *ts_data)
{
	int i;
	int len=0;
	len=sizeof(ic_key_value_array) / sizeof(ic_key_value_array[0]);
	for(i = 0; i < len ; i++){
		if(strstr(ic_key_value_array[i].ic_name , ts_data->pdata->ic_name)) {
			ts_data->fts_chip_type = ic_key_value_array[i].value;
			 FTS_ERROR("IC  name matches successfully!");
			return ;
		}
	}
	FTS_ERROR("IC  name matches failed!");
}
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    const char *name;
    FTS_FUNC_ENTER();
    ret = of_property_read_string(np, "focaltech,ic_type", &name);
	if (ret < 0) {
		pdata->ic_name = NULL;
		FTS_ERROR("IC name not obtained! please check devicetree! ");
		return -EINVAL;
	}
	else {
		pdata->ic_name = name;
		FTS_ERROR(" ic_name=%s ",pdata->ic_name);
	}
                                ...
                                unsigned long event, void *data)
{
	...
	pr_err("wyx--1-blank:%d---event=%ld-\n",blank,event);

#if 0
	if (evdata->data && event == DRM_PANEL_EVENT_BLANK && fts_data) {
		blank = *(int *) (evdata->data);
		pr_err("wyx---blank:%d--\n",blank);
		switch (blank) {
			case DRM_PANEL_BLANK_POWERDOWN:
				pr_err("wyx---------DRM_PANEL_BLANK_POWERDOWN-------\n");
					fts_ts_suspend(&fts_data->client->dev);
				break;
			case DRM_PANEL_BLANK_UNBLANK:
				pr_err("wyx---------DRM_PANEL_BLANK_UNBLANK-------\n");
...
#endif
	if (evdata->data && event == DRM_PANEL_EVENT_BLANK && fts_data) {
		pr_err("wyx--resume--blank:%d--\n",blank);
			pr_err( "%s: fts notifier  4!\n",__func__);
		switch (blank) {
			case DRM_PANEL_BLANK_UNBLANK:
				pr_err("wyx---------DRM_PANEL_BLANK_UNBLANK-------\n");
					fts_ts_resume(&fts_data->client->dev);
	...
	if (evdata->data && event == DRM_PANEL_EARLY_EVENT_BLANK && fts_data) {
	pr_err("wyx--suspend--blank:%d--\n",blank);
	switch (blank) {
		case DRM_PANEL_BLANK_POWERDOWN:
			pr_err("wwww-------DRM_PANEL_BLANK_POWERDOWN-------\n");
				fts_ts_suspend(&fts_data->client->dev);
			break;
		default:
			break;
	...
	if (!evdata || evdata->id != 0)
		return 0;
		pr_err( "%s: fts notifier begin 2!\n",__func__);
    if (evdata && evdata->data && event == MSM_DRM_EVENT_BLANK) {
             blank = evdata->data;
        if (*blank ==  MSM_DRM_BLANK_UNBLANK){
            pr_err("MSM_DRM_BLANK_UNBLANK \n");
            fts_ts_resume(&fts_data->client->dev);
        }
    }
    ...
    if (evdata && evdata->data && event == MSM_DRM_EARLY_EVENT_BLANK) {
        ...
        if (*blank == MSM_DRM_BLANK_POWERDOWN){
            pr_err("MSM_DRM_BLANK_POWERDOWN \n");
            fts_ts_suspend(&fts_data->client->dev);
...
static int check_dt(struct device_node *np)
{
	...
	pr_err("wyx------check_dt---start---\n");
	count = of_count_phandle_with_args(np, "panel", NULL);
	pr_err("wyx------check_dt---start--count=%d-\n",count);
	if (count <= 0)
		return OK;
	pr_err("wyx------check_dt---1---\n");
	...
	for (i = 0; i < count; i++) {
		...
		of_node_put(node);
		if (!IS_ERR(panel)) {
			pr_err("wyx------active_panel ok------\n");
			active_panel = panel;
			return OK;
		}
	}
	pr_err("wyx------check_dt---end---\n");
	return -ENODEV;
}
...
static int check_default_tp(struct device_node *dt, const char *prop)
{
	...
	pr_err("wyx-------%s----active_tp=%s-----compatible=%s--\n",__func__,active_tp,compatible);
	if (start == NULL) {
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    if (ts_data->dev->of_node) {
     ...
     if (check_dt(dp)) {
       if (!check_default_tp(dp, "qcom,spi-touch-active")){
                    ret = -EPROBE_DEFER;
                    pr_err("w--------spi-touch-active error \n");
                 }
                else {
                    ret = -ENODEV;
                    pr_err("w------check_dt error\n");
                }
			return ret;
        }
      #endif
    ...
    get_ic_chip_type(ts_data);
    ts_data->ic_info.is_incell =ts_data->ic_info.is_incell = ((ts_data->fts_chip_type & FLAGBIT(FLAG_IDC_BIT)) == FLAGBIT(FLAG_IDC_BIT));
    ts_data->ic_info.hid_supported =ts_data->ic_info.hid_supported = ((ts_data->fts_chip_type & FLAGBIT(FLAG_HID_BIT)) == FLAGBIT(FLAG_HID_BIT));
...
//#if (!FTS_CHIP_IDC)
	...
	FTS_ERROR("msm_drm_register_client");
	if (ret)
		FTS_ERROR("msm_drm_register_client fail: %d", ret);
    ...
    ft_fw=fw_file[270];
	pr_err("wzx --------ft_fw=%d",ft_fw);
	if(strstr(lcd_info_pr,"ft8756") !=NULL){
	   snprintf(lcdname, sizeof(lcdname),"%s ","ft8756" );
	   pr_err("lcdname\n ");
	}
	snprintf(version, sizeof(version),"0x%x ","03" );
	snprintf(vendor_name, sizeof(vendor_name),"tcl_ft8756" );
...
#if defined(ONTIM_DRM_PANEL)
        ret= drm_panel_notifier_unregister(active_panel,&ts_data->fb_notif);
        if (ret) {
            FTS_ERROR("[FB]Unable to register fb_notifier: %d", ret);
        }

    #else
       ret = msm_drm_unregister_client(&ts_data->fb_notif);
          if (ret) {
            FTS_ERROR("[FB]Unable to register fb_notifier: %d", ret);
          }
#endif
